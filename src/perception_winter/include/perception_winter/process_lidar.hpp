...
/**
 * @brief Accuracy Metrics Tracker
 */
class AccuracyMetrics {
public:
    struct Metrics {
        long true_positives_yellow = 0;
        long false_positives_yellow = 0;
        long true_positives_blue = 0;
        long false_positives_blue = 0;
    };
    
    Metrics& getMetrics() { return metrics_; }
    void updateMetrics(bool true_is_yellow, bool predicted_is_yellow);
    void printReport(rclcpp::Logger logger) const;
...
public:
    ...
    // Method without accuracy metrics
    void detectConesInClusters(const std::vector<Cluster>& clusters, 
                              std::vector<Point3D>& positions, 
                              std::vector<int>& colors,
                              ConeClassifier& classifier);
...
private:
    ...
    // Publishers
    void publishFilteredPoints(const PointCloudPtr cloud);
